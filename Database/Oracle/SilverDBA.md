...
- [要注意！落とし穴リスト](#要注意落とし穴リスト)
  - [4章 リスナー, サービス登録](#4章-リスナー-サービス登録)
    ...
    - [共有サーバ接続を使うためにはSHARED\_SERVERS初期化パラメータを設定する](#共有サーバ接続を使うためにはshared_servers初期化パラメータを設定する)
  - [5章 権限の管理](#5章-権限の管理)
  ...
  - [9章 Oracle Loader, Oracle Data Pump, 外部表](#9章-oracle-loader-oracle-data-pump-外部表)
    - [エクスプレスモードを使用しない時に生成される、もしくは使用されるファイルは聞かれる](#エクスプレスモードを使用しない時に生成されるもしくは使用されるファイルは聞かれる)
...
### 共有サーバ接続を使うためにはSHARED_SERVERS初期化パラメータを設定する
...
一方で、ロールに付与することはできない。
https://www.youtube.com/watch?v=K7UGbQWtH-A&t=3557s
## 6章 表領域の概念
...
## 9章 Oracle Loader, Oracle Data Pump, 外部表
### エクスプレスモードを使用しない時に生成される、もしくは使用されるファイルは聞かれる
・データファイル<br>
・制御ファイル<br>
・ログファイル<br>
・廃棄ファイル<br>
